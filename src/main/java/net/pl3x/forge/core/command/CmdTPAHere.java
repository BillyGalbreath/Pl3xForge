...
import net.pl3x.forge.core.util.TeleportRequestTPAHere;
...
public class CmdTPAHere extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if (args.length < 1) {
            Lang.send(player, Lang.MUST_SPECIFY_PLAYER);
            return;
        }

        EntityPlayerMP target = Utils.getPlayer(args[0]);
        if (target == null) {
            Lang.send(player, Lang.PLAYER_NOT_ONLINE);
            return;
        }

        if (Teleport.TELEPORT_REQUESTS.get(target.getUniqueID()) != null) {
            Lang.send(player, Lang.TELEPORT_REQUEST_TARGET_HAS_PENDING
                    .replace("{target}", target.getName()));
            return;
        }

        if (getPlayerData(target).denyTeleports()) {
            Lang.send(player, Lang.TELEPORT_REQUEST_TARGET_TOGGLED_OFF
                    .replace("{target}", target.getName()));
            return;
        }
        ...
        Lang.send(player, Lang.TELEPORT_REQUEST_REQUESTER
                .replace("{target}", target.getName()));
        Lang.send(target, Lang.TELEPORT_REQUEST_TPAHERE_TARGET
